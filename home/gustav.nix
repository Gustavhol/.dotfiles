{ config, pkgs, unstablePkgs, systemSettings, userSettings, inputs, system, ... }:

{
  ...
  home.packages = (with pkgs; [
    lunarvim unicode-emoji starship eza flameshot transmission_4 vscode vlc
  ]) ++ (with unstablePkgs; [
    orca-slicer beeper google-chrome obsidian rpi-imager spotify vivaldi
  ]);
    ...
    profiles = {
      "${userSettings.username}" = {
        extensions.packages = with inputs.firefox-addons.packages.${system}; [
          bitwarden ublock-origin sponsorblock decentraleyes privacy-badger clearurls
        ];
      };
    };
  };
